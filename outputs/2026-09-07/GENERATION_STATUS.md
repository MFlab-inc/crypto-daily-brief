...
call_A: OK（1回試行）
call_B: OK（1回試行）
token_usage（実消費量）: input=30745, output=5169 (call_A: in=26176 out=4533 / call_B: in=4569 out=636)
news_sources:
  - SEC: ok（対象日0件／取得25件）
  - FRB: ok（対象日0件／取得20件）
...
tier3候補 35件中 19件を選定（16件を件数上限により除外）
...
月次累計（2026-09）: input=344522, output=58687（outputs/token_usage_log.csv集計・同日複数回実行分を含む）

...
call_A: OK（1回試行）
call_B: OK（1回試行）
token_usage（実消費量）: input=29428, output=4592 (call_A: in=24856 out=4013 / call_B: in=4572 out=579)
news_sources:
  - SEC: ok（対象日3件／取得25件）
  - FRB: ok（対象日0件／取得20件）
...
tier3候補 37件中 20件を選定（17件を件数上限により除外）
独立2媒体ペア救済: 3組（5件を上限外で追加）
tier4候補 45件中 10件を選定（35件を件数上限により除外）
...
月次累計（2026-09）: input=29428, output=4592（outputs/token_usage_log.csv集計・同日複数回実行分を含む）

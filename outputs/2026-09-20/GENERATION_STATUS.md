...
call_A: OK（1回試行）
call_B: OK（1回試行）
token_usage（実消費量）: input=23883, output=3047 (call_A: in=19465 out=2566 / call_B: in=4418 out=481)
news_sources:
  - SEC: ok（対象日0件／取得25件）
  - FRB: ok（対象日0件／取得20件）
...
月次累計（2026-09）: input=863894, output=141015（outputs/token_usage_log.csv集計・同日複数回実行分を含む）

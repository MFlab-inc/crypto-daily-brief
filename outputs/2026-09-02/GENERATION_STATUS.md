...
call_A: OK（1回試行）
call_B: OK（1回試行）
token_usage（実消費量）: input=32562, output=5702 (call_A: in=27296 out=5045 / call_B: in=5266 out=657)
news_sources:
  - SEC: ok（対象日0件／取得25件）
  - FRB: ok（対象日0件／取得20件）
...
tier3候補 37件中 15件を選定（22件を件数上限により除外）
...
月次累計（2026-09）: input=94642, output=15880（outputs/token_usage_log.csv集計・同日複数回実行分を含む）

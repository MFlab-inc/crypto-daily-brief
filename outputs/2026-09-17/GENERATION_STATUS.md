...
call_A: OK（1回試行）
call_B: OK（1回試行）
token_usage（実消費量）: input=35086, output=5840 (call_A: in=29810 out=5169 / call_B: in=5276 out=671)
news_sources:
  - SEC: ok（対象日1件／取得25件）
  - FRB: ok（対象日0件／取得20件）
...
tier3候補 40件中 20件を選定（20件を件数上限により除外）
...
月次累計（2026-09）: input=770913, output=127224（outputs/token_usage_log.csv集計・同日複数回実行分を含む）

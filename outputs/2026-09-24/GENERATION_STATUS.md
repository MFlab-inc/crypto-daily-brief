...
call_A: OK（1回試行）
call_B: OK（1回試行）
token_usage（実消費量）: input=32693, output=5804 (call_A: in=27411 out=5180 / call_B: in=5282 out=624)
news_sources:
  - SEC: ok（対象日0件／取得25件）
  - FRB: ok（対象日2件／取得20件）
...
tier3候補 43件中 16件を選定（27件を件数上限により除外）
...
月次累計（2026-09）: input=993321, output=163379（outputs/token_usage_log.csv集計・同日複数回実行分を含む）

"""診断用一時スクリプト（v1.43実データ検証・オーナー指示）。

8/25データで4分岐マトリクス再構成後の挙動を検証する。
1) part1_headline に値動きの記述が入ること（数値はテンプレート差し込み、
   LLMは形状のみ）
2) part1_points にニュース材料が確認できなかった旨の項目が残ること
3) C16b・C22を含む全監査がPASSすること
...
コミットはしない。調査後、本スクリプトとワークフローは削除する。
"""
import json
import sys

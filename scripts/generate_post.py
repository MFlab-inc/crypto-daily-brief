...
NO_CANDIDATES_FALLBACK = f"""## part1_headline・part1_pointsの決定（v1.42・オーナー指示）

part1_headline および part1_points は、次の2つを独立に確認して
決定する。
(i) 採用に足るニュース材料があるか
(ii) 入力の intraday_range に notable_move: true の銘柄があるか

A. (i)あり・(ii)あり → 両方を記述する。ヘッドラインは重要度の
   高い方を主とし、他方を従として併記する。
B. (i)あり・(ii)なし → 従来どおりニュース材料を記述する。
C. (i)なし・(ii)あり → 値動きを記述する。定型文は使わない。
   ただし、ニュース材料が確認できなかった旨は主要なポイントに
   1項目として明記する。
D. (i)なし・(ii)なし → 統合運用基準§3.1の定型文を使う。

CとDを取り違えないこと。定型文を使うのはDの場合のみである。

（この A〜D は本文の構成方針を表す4分岐であり、下記audit_ledgerの
reasonで使うA/B/C——個々の候補材料の重要性判定（「重要性判定と
因果表現の分離」参照）——とは別の分類である。混同しないこと。）

### (i)「採用に足るニュース材料があるか」の判定基準

tier 1の裏付けがある採用材料が1件以上ある場合、または上記「独立2ソース
規定」に該当するtier3材料がある場合、(i)は「あり」である。ただし
独立2ソース材料のみでtier1裏付けが無い場合、part1_pointsにはその材料を
記載してよいが、part1_headlineは独立2ソース材料だけでは昇格しない
（上記「ヘッドラインの判定手順」のとおり）——この場合のヘッドラインは、
(ii)が「あり」ならC同様に値動きを、(ii)も「なし」ならDの定型文を使う。
tier1の裏付けがある採用材料が無く、かつ独立2ソース規定に該当する
tier3材料も無い場合にのみ、(i)は「なし」である。

### C（(i)なし・(ii)あり）の詳細

上記「日中の値動き（notable_move）」の指示に従い、値動きの形状のみを
記述する（数値は書かない・後段のテンプレートが差し込む）。
part1_headlineはこの値動きの記述とし、下記D用の定型文は使わない。
...
- headline_for_image: Dの場合（ニュース・値動きいずれの材料も無い場合）は、
  daily_data.json内のBTC・ETHのdirection（up/down）に基づく短い定性的な
  見出しにとどめる（例:「BTC・ETHともに上昇基調」）。数値は書かない。
  `#`は使わず全角40字以内。
